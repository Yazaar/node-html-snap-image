...
describe('beforeScreenshot', () => {
  ...
  it('should compile a screenshot if there is content', async () => {
    ...
    expect(page.setContent).toHaveBeenCalledWith('<html><body>Hello world!</body></html>', expect.anything());
  });
  ...
  it('should not compile a screenshot if content is empty', async () => {
    ...
    expect(page.setContent).toHaveBeenCalledWith('<html><body>{{message}}</body></html>', expect.anything());
  });
  ...
  it('should use full-page screenshot options when clip.full is true', async () => {
    const pageWithFullCapture = {
      setContent: jest.fn(),
      setDefaultTimeout: jest.fn(),
      $: jest.fn(() => ({ screenshot: jest.fn(() => buffer) })),
      screenshot: jest.fn(() => buffer)
    };

    await makeScreenshot(pageWithFullCapture, {
      clip: { full: true },
      screenshot: new Screenshot({
        html: '<html><body>Hello world!</body></html>'
      })
    ...
    expect(pageWithFullCapture.screenshot).toHaveBeenCalledWith(
  ...
  it('should compute clip coordinates from the element bounds when clip.points is provided', async () => {
    const element = {
      screenshot: jest.fn(() => buffer),
      boundingBox: jest.fn(() => ({ x: 10, y: 20, width: 30, height: 40 }))
    };
    const pageWithClip = {
      setContent: jest.fn(),
      setDefaultTimeout: jest.fn(),
    ...
    await makeScreenshot(pageWithClip, {
      clip: {
        points: {
          x: 5,
          y: 2,
...
describe('handlebarsHelpers', () => {
  ...
  beforeEach(() => {
    ...
    if (Object.prototype.hasOwnProperty.call(handlebars.helpers, 'equals') && !!handlebars.helpers.equals) {
  ...
  describe('if logic is given in the template', () => {
    ...
    for (const test of validTests) {
      ...
      it(`if logic is given in the template, it should render the expected template when ${test.label}`, async () => {
        ...
        expect(p().setContent).toHaveBeenCalledWith(test.expectedHtml, expect.anything());
      });
    }
  });
  ...
  describe('functional tests', () => {
    it('should do conditional rendering correctly', async () => {
      ...
      expect(page.setContent).toHaveBeenCalledWith('<html><body><div>Hello world!</div></body></html>', expect.anything());
    });
    ...
    it('should do conditional rendering on an array correctly', async () => {
      ...
      expect(page.setContent).toHaveBeenCalledWith('<html><body><div>Hi!</div><div>Hello!</div></body></html>', expect.anything());
    });
    ...
    it('should do conditional rendering with unless correctly', async () => {
      ...
      expect(page.setContent).toHaveBeenCalledWith('<html><body><div>Not hello world!</div></body></html>', expect.anything());
    });
    ...
    it('should do conditional rendering with unless on an array correctly', async () => {
      ...
      expect(page.setContent).toHaveBeenCalledWith("<html><body><div>I'm not hidden!</div></body></html>", expect.anything());
    });
    ...
    it('should transform a string correctly', async () => {
      ...
      expect(page.setContent).toHaveBeenCalledWith('<html><body><div>HI THERE</div></body></html>', expect.anything());
    });
    ...
    it('should transform a number correctly', async () => {
      ...
      expect(page.setContent).toHaveBeenCalledWith('<html><body><div>7</div></body></html>', expect.anything());
    });
    ...
    it('should replace a character correctly', async () => {
      ...
      expect(page.setContent).toHaveBeenCalledWith('<html><body><div>This is bar</div></body></html>', expect.anything());
    });
  });
});
